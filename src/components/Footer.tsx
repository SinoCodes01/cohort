export default function Footer() {
  const cols = [
    { title: "Product", links: ["Features", "Communities", "Events", "Pricing"] },
    { title: "Company", links: ["About", "Blog", "Careers", "Press"] },
    { title: "Support", links: ["Help Center", "Contact", "Privacy", "Terms"] },
  ];

  return (
    <footer style={{ background: "white", borderTop: "1px solid #E5E7EB", padding: "3rem 1.5rem" }}>
      <div style={{ maxWidth: "1280px", margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "2rem", marginBottom: "2.5rem" }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem" }}>
              {[
                ...
                <a key={social.label} href="#" aria-label={social.label} style={{ width: "2.25rem", height: "2.25rem", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: "#F3F4F6", color: "#6B7280", transition: "all 0.2s" }}>
                  <svg style={{ width: "16px", height: "16px" }} fill="currentColor" viewBox="0 0 24 24">{social.icon}</svg>
                </a>
              ))}
            </div>
          </div>
          ...
          {cols.map((col) => (
            <div key={col.title}>
              <h4 style={{ fontWeight: 600, color: "#111827", marginBottom: "0.75rem", fontSize: "0.875rem" }}>{col.title}</h4>
              <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {col.links.map((link) => (
                  <li key={link}>
                    <a href="#" style={{ color: "#6B7280", fontSize: "0.875rem", textDecoration: "none", transition: "color 0.2s" }}>{link}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div style={{ borderTop: "1px solid #E5E7EB", paddingTop: "2rem", display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: "1rem" }}>
          <p style={{ color: "#6B7280", fontSize: "0.875rem" }}>© 2026 Cohort, Inc. All rights reserved.</p>
          <p style={{ color: "#9CA3AF", fontSize: "0.75rem" }}>Made with ❤️ for students everywhere</p>
        </div>
      </div>
    </footer>
  );
